...
- [x] **Spike do mecanismo de instrumentação C# — decidido: `netcoredbg` via DAP.** ICorDebug via interop nem chegou a ser tentado (`netcoredbg` já se mostrou viável e é bem menos trabalhoso que driblar uma API COM nativa).
  ...
  - [ ] **Trabalho de acabamento restante** (menor risco, mesma família de interfaces já dominada):
    - Dereferenciar `ICorDebugObjectValue`/`ICorDebugArrayValue` (objetos com campos, arrays) — mesmo padrão do `ICorDebugStringValue`, IID já obtido (`ICorDebugArrayValue` = `0405B0DF-A660-11D2-BD02-0000F80849BD`, confirmado no `cordebug.idl`), falta implementar a vtable
    - Nomes de variáveis locais (`IMetaDataImport` não tem isso — nomes de variável local só existem no PDB, diferente de nomes de método/tipo que vêm da assembly principal)
    - Mapear IL offset → número de linha do C# original via PDB (`ISymUnmanagedReader` ou parsing de portable PDB — ainda nem investigado qual mecanismo o CoreCLR expõe no Linux)
  - [ ] Ainda não testado: memória crescente, referência circular, multi-thread, timeout de execução combinado com o nsjail — bateria equivalente à que fizemos pro Java na Fase 0.5
