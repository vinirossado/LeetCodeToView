...
>
> **Decisão fechada: sem scheduling de cluster por enquanto.** Deploy atual é um único host (VPS), 1 instância do `api` — hoje gerenciada por uma unit systemd (`.ci/leetcodeview-api.service`), não mais um serviço Swarm replicável (ver "Deploy real (VPS)" acima) — deliberadamente única: a superfície de privilégio do nsjail não deve ser multiplicada até essa questão ter uma resposta melhor. Bin-packing/scheduling de cluster é, por definição, um problema de MÚLTIPLOS hosts — não existe hoje. Mesmo racional de "não pagar a complexidade antes de precisar" do item de IPC acima. **Gatilho pra revisitar**: escalar horizontalmente além de um único host.
...
### Deploy real (VPS): `api` roda fora do modelo de serviço do Docker Swarm (decisão fechada, implementada)
...
**Solução implementada**: o container `api` roda como um `docker run` comum, gerenciado por uma unit systemd (`.ci/leetcodeview-api.service`), FORA do modelo de serviço do Swarm — não porque o Swarm em si seja abandonado (o `frontend`, que não precisa desses privilégios, continua um serviço Swarm normal), mas porque só esse serviço específico precisa deles. O container se conecta à MESMA rede overlay `proxy_net` (criada com `--attachable`, pra permitir que um container solto participe dela) que os serviços Swarm já usam — confirmado empiricamente que um serviço Swarm de verdade consegue resolver e alcançar esse container pelo nome DNS na mesma rede, então um reverse proxy rodando como serviço Swarm continua roteando pra ele normalmente. `deploy.sh` foi reescrito pra `systemctl restart` em vez de `docker stack deploy`.
