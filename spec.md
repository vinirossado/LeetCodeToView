...
>
> **PDB investigado, depois implementado via parser próprio.** O SDK do .NET não tem nenhuma lib nativa de symbol reader (`ISymUnmanagedReader`), e `libdbgshim.so` não exporta nada de PDB. O próprio `netcoredbg` carrega Roslyn (`Microsoft.CodeAnalysis.CSharp.dll`) + um `ManagedPart.dll` — evidência de que leitura de Portable PDB moderna é via código gerenciado (`System.Reflection.Metadata`), não COM nativo. Decisão original: não seguir por esse caminho — seria escrever um parser de Portable PDB do zero (domínio de parsing binário, diferente do padrão de interop COM usado no resto do C#). Essa decisão foi revisitada: `sandbox/src/pdb.rs` implementa um parser mínimo (lê só as tabelas `Document`/`MethodDebugInformation`/`LocalScope`/`LocalVariable` do metadata físico do `.pdb`, o suficiente pra resolver índice de slot → nome real, dado um token de método + offset IL), validado com um `.pdb` real gerado por `dotnet build -c Debug` (fixture em `sandbox/src/testdata/`, testes em `pdb.rs`). **Limitação restante, não bloqueante**: ainda não há número de linha do C# original — os `SequencePoints` (blob de `MethodDebugInformation`) que dariam offset IL → linha não foram implementados, só o mapeamento de nomes de variável; `line` continua sendo o offset de IL. O pipeline de debug do C# está completo pra tudo além disso: attach, ciclo de vida, breakpoint, step, extração de locals (primitivos, string, array) com nomes reais, call stack com nome de método.
